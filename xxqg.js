...
var aCount = 8; //文章默认学习篇数
var vCount = 6; //小视频默认学习个数
var cCount = 2; //收藏+分享+评论次数

var aTime = 65; //每篇文章学习-103秒 103*7≈720秒=12分钟
var vTime = 15; //每个小视频学习-15秒
var rTime = 380; //广播收听-6分钟
var firstRun = true;
var commentText = ["支持党，支持国家！", "为实现中华民族伟大复兴而不懈奋斗！", "紧跟党走，毫不动摇！",
    "不忘初心，牢记使命", "努力奋斗，报效祖国！"
]; //评论内容，可自行修改，大于5个字便计分

var aCatlog = "要闻" //文章学习类别，可自定义修改为“要闻”、“新思想”等
...
function articleStudy() {
    while (!text("强国通").exists()); //等待加载出主页
    click(500,2000); //点击主页正下方的"学习"按钮
    delay(2);
 ...
 * @return: null
 */
function videoStudy_bailing_time() {
    h = device.height; //屏幕高
    w = device.width; //屏幕宽
    x = (w / 3) * 2; //横坐标2分之3处
    h1 = (h / 6) * 5; //纵坐标6分之5处
...
function videoStudy_bailing_count() {
    h = device.height; //屏幕高
    w = device.width; //屏幕宽
    x = (w / 3) * 2; //横坐标2分之3处
    h1 = (h / 6) * 5; //纵坐标6分之5处
...
function getScores() {
    ...
    if (firstRun) {
        aCount = 8 - parseInt((myScores["我要选读文章"]) / 2);
    } else {
        aCount = 6 - parseInt((myScores["我要选读文章"]) / 2);
    }
    aTime = 66;
    vCount = 6 - myScores["视听学习"];
    rTime = (6 - myScores["视听学习时长"]) * 65;
 ...
 */
function start_app() {
    shell("pm enable cn.xuexi.android", true);
...
function main() {
    ...
    /*if (vCount != 0) {
        videoStudy_bailing_count(); //看视频
    }*/
    if (rTime != 0) {
        videoStudy_bailing_time();
        //listenToRadio(); //听电台广播
    }
